...
template<std::floating_point T> struct shichi_data;

// From shichif.c
template<> struct shichi_data<float> {
  static constexpr std::array<float, 9> S1 = {
    -3.56699611114982536845E-8, 1.44818877384267342057E-7,  7.82018215184051295296E-7,
  ...
  static constexpr std::array<float, 10> C1 = {
    1.31458150989474594064E-8,  -4.75513930924765465590E-8, -2.21775018801848880741E-7,
  ...
  static constexpr std::array<float, 7> S2 = {
    1.69050228879421288846E-8, 1.25391771228487041649E-7, 1.16229947068677338732E-6,
    1.61038260117376323993E-5, 3.49810375601053973070E-4, 1.28478065259647610779E-2,
    1.03665722588798326712E0,
  };
  static constexpr std::array<float, 7> C2 = {
    -3.00095178028681682282E-9, 7.79387474390914922337E-8, 1.06942765566401507066E-6,
...
// From shichi.c
template<> struct shichi_data<double> {
  static constexpr std::array<double, 22> S1 = {
  ...
  static constexpr std::array<double, 23> C1 = {
  ...
  static constexpr std::array<double, 23> S2 = {
  ...
  static constexpr std::array<double, 24> C2 = {
...
/**
 * @brief Hyperbolic sine and cosine integrals
 * * @tparam T floating point type (float or double)
 * @param x Input argument
 * @param shi [out] Hyperbolic sine integral
 * @param chi [out] Hyperbolic cosine integral
 */
template<std::floating_point T> void shichi(T x, T& shi, T& chi) {
  using namespace internal;

  constexpr T EUL = std::numbers::egamma_v<T>;
  constexpr T MACHEP = std::numeric_limits<T>::epsilon();
  ...
  if (abs_x < T(8)) {
    ...
    do {
      ...
      s += a / k;
      k += T(1);
    } while (std::abs(a) > MACHEP * std::abs(s));

    s *= abs_x;
  } else {
    T k = std::exp(abs_x) / abs_x;
    T a;

    if (abs_x < T(18)) {
      a = (T(576) / abs_x - T(52)) / T(10);
      s = k * chbevl(a, shichi_data<T>::S1);
      c = k * chbevl(a, shichi_data<T>::C1);
    } else if (abs_x <= T(88)) {
      a = (T(6336) / abs_x - T(212)) / T(70);
      s = k * chbevl(a, shichi_data<T>::S2);
      c = k * chbevl(a, shichi_data<T>::C2);
    } else {
