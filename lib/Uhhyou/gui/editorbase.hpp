...
public:
  EditorBase(
    ...
    , pluginInfoButton(
        *this, palette, navManager, processor.getName(), infoText, licenseText)
    , undoButton(
        *this,
        palette,
        statusBar,
    ...
    , randomizeButton(
        *this, palette, statusBar, numberEditor, "Randomize", randomizeButtonOnClick)
  ...
  void paintOverChildren(juce::Graphics &ctx) override
  {
    ...
    auto drawFocusIfActive = [&](juce::Component &cmp)
    {
      float thickness = palette.borderThin() * float(3);
      auto bounds
        = getLocalArea(&cmp, cmp.getLocalBounds()).toFloat().expanded(thickness);
      ctx.setColour(palette.foreground().withAlpha(float(0.5)));
      ctx.drawRect(bounds, thickness);
    };

    if (!hasKeyboardFocus(false)) {
      if (auto *focused = juce::Component::getCurrentlyFocusedComponent()) {
        if (mainScope.contains(focused)) {
          drawFocusIfActive(*focused);
        }
      }
    }
...
protected:
  ...
  // Registers a component for interaction (Focus, Shadows, Mouse events).
  ...
  void addRawWidgetToSection(
    const juce::String &sectionTitle, std::shared_ptr<juce::Component> widget)
  {
  ...
  void
  registerParameterId(const juce::String &id, const juce::AudioProcessorParameter *prm)
  {
  ...
  {
    auto widget = std::make_shared<LabeledWidget>(
      paramLocks, palette, label, component, parameter, option);

    addAndMakeVisible(*widget);
    addRawWidgetToSection(sectionTitle, widget);

    registerInteractive(component);
    if (option == LabeledWidget::showLabel) {
      registerInteractive(widget->lockableLabel());
    }
  }

  template<Style style = Style::common, typename Scale>
  auto addMomentaryButton(
    ...
    LabeledWidget::Layout layout = LabeledWidget::expand)
  {
    ...
    return addButton_<ButtonType>(section, paramId, scale, label, layout);
  }
    ...
    LabeledWidget::Layout layoutOption = LabeledWidget::showLabel)
  {
    ...
    return addKnob_<KnobType>(
      sectionTitle, paramId, scale, snaps, precision, label, layoutOption);
  }
    ...
    juce::String labelY = "")
  {
    ...
    auto pad = std::make_shared<XYPad>(
    ...
    auto wrapper = std::make_shared<LabeledXYPad>(
      ...
      [this, snapKey](bool state)
      { getStateTree().setProperty(snapKey, state, nullptr); });
...
private:
    ...
    LabeledWidget::Layout layoutOption)
  {
    ...
    auto widget = std::make_shared<KnobType>(
      *this, palette, &processor.undoManager, prm, scale, statusBar, numberEditor,
      precision);
  ...
  auto addButton_(
    ...
    Scale &scale,
    juce::String label,
    LabeledWidget::Layout layout)
  {
    auto prm = processor.param.tree.getParameter(paramId);
    if (label.isEmpty() && prm) label = prm->getName(2048);

    auto widget = std::make_shared<ButtonType>(
