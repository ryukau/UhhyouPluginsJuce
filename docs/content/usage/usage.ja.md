...
使い方で何かあれば [GitHub で discussion を開く](https://github.com/ryukau/UhhyouPluginsJuce/discussions)か `ryukau@gmail.com` にメールしてください。お気軽にどうぞ。
...
`.zip`ファイルを展開し、 `assets` フォルダの中にある `UhhyouPlugins` を以下のフォルダに移動してください。

| OS               | パス                                           |
|------------------|----------------------------------------------|
| Windows          | `%APPDATA%\`                                 |
| macOS            | `~/Library/Preferences/`                     |
| Linux (Primary)  | `$XDG_CONFIG_HOME/`                          |
| Linux (Fallback) | `$HOME/.config/` (`$XDG_CONFIG_HOME` が空のとき) |

設定フォルダはプラグインを初めて開いたときに自動的に作成されます。
...
| Linux (Fallback) | `$HOME/.config/UhhyouPlugins/$PluginName/presets` (`$XDG_CONFIG_HOME` が空のとき) |

プリセットは [XML](https://www.mlit.go.jp/plateaudocument01-04/1.html) です。テキストエディタなどでの編集は非推奨です。書式、パラメータ名、値などにエラーがあるとプラグインがホストを巻き込んでクラッシュすることがあります。
