...
If you have troubles related to the plugins, feel free to contact on [GitHub Discussions page](https://github.com/ryukau/UhhyouPluginsJuce/discussions) or `ryukau@gmail.com.`
...
Extract the `.zip` file and move the `UhhyouPlugins` folder in `assets` to these folders.
...
The plugin will automatically create a settings folder the first time you open it.
...
| OS               | Path                                                                               |
|------------------|------------------------------------------------------------------------------------|
| Windows          | `%APPDATA%\UhhyouPlugins\$PluginName\presets`                                      |
| macOS            | `~/Library/Application Support/UhhyouPlugins/$PluginName/presets`                  |
| Linux (Primary)  | `~/.config/UhhyouPlugins/$PluginName/presets`                                      |
| Linux (Fallback) | `$HOME/.config/UhhyouPlugins/$PluginName/presets` (If `$XDG_CONFIG_HOME` is empty) |

Presets are [XML](https://www.w3schools.com/xml/default.asp). Avoid editing presets on text editors. Syntax errors, wrong parameter names, or out of range values may crash the plugins and their host application.
