...
## Plugin Location
Extract the downloaded `.zip` file and move the plugin files to a following path.
...
If the downloaded package includes `presets` folder, it must be installed  manually. Move the `Uhhyou` folder in `presets` into the following path:
...
### Windows
If the plugin is not recognized by DAW, it may be required to install the latest Microsoft C++ Redistributable.
...
### macOS
Because the plugins are released without an Apple Developer signature, Gatekeeper may flag them as damaged.
...
Note: `xattr` and `codesign` are built into macOS. If this is the first time using `codesign`, Mac may prompt you to download the "Command Line Developer Tools." Simply click "Install" to proceed.
