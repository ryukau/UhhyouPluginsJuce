...
public:
  MeterDisplay(
    Component &parent,
    Palette &palette,
    std::array<std::atomic<float>, nChannel> &peakAmplitude,
  ...
  virtual void resized() override
  {
    ...
    stroke.setStrokeThickness(pal.borderThin());
  }
  ...
  virtual void paint(juce::Graphics &ctx) override
  {
    ...
    ctx.drawText(labelTitle, labelBounds, juce::Justification::centred);
    ...
    for (size_t i = 0; i < nChannel; ++i) {
      ...
      ctx.drawText(
        std::format("{:+3.1f} dB", meter[i].holdDecibel), area,
    ...
    for (float db = minDecibel; db <= maxDecibel; db += tickStepDecibel) {
      const float bottom
        = tickBounds.getY() + decibelToHeight(db, tickBounds.getHeight());

      ctx.drawLine(
        tickBounds.getRight(), bottom, fullBounds.getWidth(), bottom,
        float(0.25) * pal.borderThin());
    ...
    juce::ColourGradient gradient{
      pal.highlightButton(), meterBounds.getBottomLeft(), pal.highlightWarning(),
      meterBounds.getTopLeft(), false};
    gradient.addColour(std::nextafter(float(0.5), 0.0), pal.highlightButton());
    gradient.addColour(float(0.5), pal.highlightWarning());
    ctx.setGradientFill(gradient);

    auto channelArea = meterBounds;
    for (size_t i = 0; i < nChannel; ++i) {
      auto barRect = channelArea.removeFromLeft(channelWidth);
      barRect.reduce(pal.borderThin(), 0);
    ...
    for (size_t i = 0; i < nChannel; ++i) {
      ...
      const float holdH = meterBounds.getY()
        + meterBounds.getHeight() * (float(1) - holdN) + float(0.5) * pal.borderThin();
      ...
      ctx.drawLine(holdL, holdH, holdR, holdH, pal.borderThin());
      holdL = holdR;
    }
  }
};
