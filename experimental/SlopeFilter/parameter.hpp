  ...
  auto constructParameter() {
    ...
    value.startHz = addParameter(
      ...
      std::make_unique<ScaledParameter<Scales::DecibelScl>>(
        0.0f, scale.startHz, "startHz", "Start", Cat::genericParameter, version0, "Hz", Rep::raw));
    value.slopeDecibel
      = addParameter(generalGroup,
                     std::make_unique<ScaledParameter<Scales::LinearScl>>(
                       scale.slopeDecibel.invmap(0.0f), scale.slopeDecibel, "slopeDecibel", "Slope",
                       Cat::genericParameter, version0, "dB", Rep::raw));
    value.outputGain
      = addParameter(generalGroup,
                     std::make_unique<ScaledParameter<Scales::DecibelScl>>(
                       scale.outputGain.invmapDB(0.0f), scale.outputGain, "outputGain",
                       "Output Gain", Cat::genericParameter, version0, "dB", Rep::display));

    value.shelvingType = addParameter(generalGroup,
                                      std::make_unique<ScaledParameter<Scales::UIntScl>>(
                                        scale.boolean.invmap(1), scale.boolean, "shelvingType",
